...
def text_node_to_html_node(text_node: TextNode) -> LeafNode:
    if text_node.text_type == TextType.TEXT:
        ...
        return LeafNode(tag='img', value=None, props={'src': text_node.url, 'alt': text_node.text} if text_node.url else None)
    else:
        raise Exception(f"Unsupported text type: {text_node.text_type.value}")
